...
def get_base_lang_config(lang: str):
    ...
    sqlmodel_url_base = "https://sqlmodel.tiangolo.com/"
    new_config = en_config.copy()
    new_config["site_url"] = en_config["site_url"] + f"{lang}/"
    new_config["theme"]["logo"] = sqlmodel_url_base + en_config["theme"]["logo"]
    new_config["theme"]["favicon"] = sqlmodel_url_base + en_config["theme"]["favicon"]
    ...
    for css in en_config["extra_css"]:
        if css.startswith("http"):
            ...
            extra_css.append(sqlmodel_url_base + css)
    ...
    for js in en_config["extra_javascript"]:
        if js.startswith("http"):
            ...
            extra_js.append(sqlmodel_url_base + js)
...
def get_sections(nav: list) -> Dict[Tuple[str, ...], str]:
    ...
    for item in nav:
        if type(item) is str:
            ...
            item_key = list(itemkdocs-markdownextradata-pluginm.keys())[0]
            sub_nav = item[item_key]
