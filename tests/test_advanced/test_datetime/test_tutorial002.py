...
def test_tutorial(print_mock: PrintMock, mod: ModuleType) -> None:
    ...
    assert print_mock.calls == [
        [
            ...
            {
                "id": None,
                "starts_at": datetime(2026, 1, 1, 12, tzinfo=timezone.utc),
                "local_time": datetime(2026, 1, 1, 12),
            },
        ]
    ]
    ...
    with pytest.raises(ValidationError) as exc_info:
        mod.Event.model_validate(
            {
                "starts_at": "2026-01-01T12:00:00",
                "local_time": "2026-01-01T12:00:00Z",
            }
        )
    errors = exc_info.value.errors()
    assert [(error["loc"], error["type"]) for error in errors] == [
        (("starts_at",), "timezone_aware"),
        (("local_time",), "timezone_naive"),
    ]

    table = SQLModel.metadata.tables["event"]
    assert table.c.starts_at.type.timezone is True
    assert table.c.local_time.type.timezone is False
