...
async def tutorial_async001_py36():
    from docs_src.advanced.asyncio import tutorial001_py36 as mod
...
def test_tutorial_001(clear_sqlmodel):
    if sys.version_info.minor >= 7:
        ...
        loop.run_until_complete(asyncio.wait([tutorial_async001()]))


def test_tutorial_001_py36(clear_sqlmodel):
    if sys.version_info.minor >= 7:
        asyncio.run(tutorial_async001_py36())
    else:
        loop = asyncio.get_event_loop()
        loop.run_until_complete(asyncio.wait([tutorial_async001_py36()]))
