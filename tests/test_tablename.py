...
def test_tablename_not_set_on_plain_model() -> None:
    """Non-table models don't get a __tablename__ injected."""

    class Schema(SQLModel):
        name: str

    assert not hasattr(Schema, "__tablename__") or not isinstance(
        Schema.__tablename__, str
    )
